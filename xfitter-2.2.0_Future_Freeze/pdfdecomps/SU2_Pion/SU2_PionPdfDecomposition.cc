/*
   @file SU2_PionPdfDecomposition.cc
   @date 2018-08-07
   @author  AddPdfDecomposition.py
   Created by  AddPdfDecomposition.py on SU2_Pion
*/
#include"xfitter_cpp_base.h"
#include<iostream>
#include"SU2_PionPdfDecomposition.h"
#include"xfitter_pars.h"
#include"xfitter_steer.h"
using namespace std;
//For dynamic loading:
namespace xfitter{
extern "C" SU2_PionPdfDecomposition*create(const char*name){
  return new SU2_PionPdfDecomposition(name);
}
SU2_PionPdfDecomposition::SU2_PionPdfDecomposition(const char*name):BasePdfDecomposition{name}{}
const char*SU2_PionPdfDecomposition::getClassName()const{return"SU2_Pion";}
...
// Init at start:
void SU2_PionPdfDecomposition::atStart(){
...
void SU2_PionPdfDecomposition::atIteration() {
  //Enforce sum rules
  //Valence sum
  par_v->setMoment(-1,2);
  //Momentum sum
...
map<int,double>SU2_PionPdfDecomposition::xfxMap(const double x)const{
  const double v=(*par_v)(x);
  const double S=(*par_S)(x);
  const double g=(*par_g)(x);
  const double u=S * (1./4.);
