...
namespace xfitter{
//for dynamic loading
extern"C" GRV_sea_PdfParam*create(const char*name){
  return new GRV_sea_PdfParam(name);
}
void GRV_sea_PdfParam::atStart(){
  using namespace std;
...
void GRV_sea_PdfParam::atIteration(){
  updateNormalization();
}
void GRV_sea_PdfParam::updateNormalization(){
  //norm=A/beta(B+1,C+1)
  const double b=*pars[1]+1;
  const double c=*pars[2]+1;
  norm=(*pars[0])*exp(-lgamma(b)-lgamma(c)+lgamma(b+c));
}
double GRV_sea_PdfParam::operator()(double x)const{
...
double GRV_sea_PdfParam::moment(int n)const{
...
void GRV_sea_PdfParam::setMoment(int n,double val){
