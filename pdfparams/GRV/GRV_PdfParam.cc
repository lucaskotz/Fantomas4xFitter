...
namespace xfitter{
//for dynamic loading
extern"C" GRV_PdfParam*create(const char*name){
  return new GRV_PdfParam(name);
}
void GRV_PdfParam::atStart(){
  using namespace std;
...
void GRV_PdfParam::atIteration(){
  updateNormalization();
}
void GRV_PdfParam::updateNormalization(){
  //norm=A/beta(B+1,C+1)
  const double b=*pars[1]+1;
  const double c=*pars[2]+1;
  norm=(*pars[0])*exp(-lgamma(b)-lgamma(c)+lgamma(b+c));
}
double GRV_PdfParam::operator()(double x)const{
...
double GRV_PdfParam::moment(int n)const{
...
void GRV_PdfParam::setMoment(int n,double val){
